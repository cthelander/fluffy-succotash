class TwoSpace extends Heap {
  ...
  private int forward(int obj) {
    ...
    // Case 1
    if( obj >= 0 )                         // Not a pointer
    {
        return obj;
    }
    // Case 2
    int len = heap[size + obj];            // Get length of object   
    if( len < 0 )                          // Object is already forwarded
    {
        return len;
    }
    // Case 3                              // Object needs to be forwarded
    for( int i = 0; i <= len ; ++i)
    {
       toSpace[hp] = heap[size + obj + i]; // Copy from heap to new heap (toSpace)
        ++hp;
    }
    heap[size + obj] = -(size - hp + len + 1);       // Set length of object to location in new heap
  ...
  private int scavenge(int obj) {
    ...
    for( int i = 0; i < len; ++i )              // Check all fields of object for pointer
    {
            temp = toSpace[obj + i];            // Get field of object
            if( temp < 0 && temp >= -size)       // Pointer needs to be forward
            {
               toSpace[obj + i] = forward(temp);
            }
    }
    return 1+len;
  }
