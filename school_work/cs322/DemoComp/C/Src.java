...
class Half extends IExpr {
  private IExpr e;
  Half(IExpr e) { this.e = e; }

  int eval(Memory mem) { 
        return e.eval(mem) / 2; }

  String show()  { return "(" + " Half " + e.show() + ")"; }

  Code compileTo(Reg reg, Code next) {
    return e.compileTo(reg,
    new HALF(reg, next));
  }
}
...
class Not extends BExpr {
  ...
  Code compileTo(Reg reg, Code next) {
    return e.compileTo(reg, new NOT(reg, next));
...
class Even extends BExpr {
  ...
  String show()  { return "(" + " Even " + e.show() + ")"; }
