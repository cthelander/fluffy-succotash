...
class NOT extends Code {
  private Reg reg;
  private Code next;
  NOT(Reg reg, Code next) {
    this.reg = reg;
    this.next = next;
  }

  void print() {
    System.out.println("  " + reg + " <- NOT " + reg );
    next.print();
  }
...
class EVEN extends Code {
  ...
  void print() {
    System.out.println("  " + reg + " <- Even " + reg );
    next.print();
  }

  Block run(Memory mem) {
    reg.setBool((reg.get() % 2) == 0);
    return next.run(mem);
  }
}

class HALF extends Code {
  private Reg reg;
  private Code next;
  HALF(Reg reg, Code next) {
    this.reg = reg;
    this.next = next;
  }

  void print() {
    System.out.println("  " + reg + " <- Half " + reg );
    next.print();
  }
