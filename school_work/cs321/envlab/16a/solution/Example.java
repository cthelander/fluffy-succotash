...
class PairExp extends Exp {
  Exp left;
  Exp right;
  PairExp(Exp left, Exp right) {
    this.left = left; this.right = right;
  }
 
  Object eval(FunctionEnv fenv, ValueEnv venv) throws RuntimeError {
    return new Pair(left.eval(fenv,venv),right.eval(fenv,venv));
  }

  Type check(FunctionEnv fenv, TypeEnv tenv) throws StaticError {
    return new PairType(left.check(fenv,tenv),right.check(fenv,tenv));
  }
}
...
class FstExp extends Exp {
  ...
  Object eval(FunctionEnv fenv, ValueEnv venv) throws RuntimeError {
    try {
      return ((Pair)(e.eval(fenv,venv))).left;
    } catch (ClassCastException exn) {
      throw new RuntimeError("Fst requires PAIR operand");
    }
  }
   
  Type check(FunctionEnv fenv, TypeEnv tenv) throws StaticError {
    Type et = e.check(fenv,tenv);
    if (et instanceof PairType)
      return ((PairType) et).left;
    else
      throw new StaticError("Fst requires PAIR operand");
  }
}
...
class SndExp extends Exp {
  ...
  Object eval(FunctionEnv fenv, ValueEnv venv) throws RuntimeError {
    try {
      return ((Pair)(e.eval(fenv,venv))).right;
    } catch (ClassCastException exn) {
      throw new RuntimeError("Snd requires PAIR operand");
    }
  }
  ...
  Type check(FunctionEnv fenv, TypeEnv tenv) throws StaticError {
    Type et = e.check(fenv,tenv);
    if (et instanceof PairType)
      return ((PairType) et).right;
    else
      throw new StaticError("Snd requires PAIR operand");
  }
}
    ...
    Program p = 
      new Program(
           new Function[] {
               new Function("eq",
                            "z",
                            new PairType(Type.INT,Type.INT),
                            Type.BOOL,
                            new LetExp("z1",
                                       new FstExp(new VarExp("z")),
                                       new LetExp("z2",
                                                  new SndExp(new VarExp("z")),
                                                  new AndExp(new LeqExp(new VarExp("z1"), 
                                                                        new VarExp("z2")),
                                                             new LeqExp(new VarExp("z2"),
                                                                        new VarExp("z1")))))),
 	      /* There is no way to define a "short-circuiting" version of OR using a function, because
 		 both operands of the pair argument are evaluated before the pair is built or the function
 		 is called. */
               new Function("or",
                            "y",
                            new PairType(Type.BOOL,Type.BOOL),
                            Type.BOOL,
                            new NotExp(new AndExp(new NotExp(new FstExp(new VarExp("y"))),
                                                  new NotExp(new SndExp(new VarExp("y"))))))},
           new CallExp("or",
                       new PairExp(new VarExp("false"),
                                   new CallExp("eq", new PairExp(new NumExp(8), 
                                                                 new AddExp(new NumExp(5), new NumExp(3)))))));
    try {
