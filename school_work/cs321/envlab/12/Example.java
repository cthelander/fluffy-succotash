...
class LesEqu extends Exp {
  Exp right;
  Exp left;
  LesEqu(Exp rigth, Exp left) {this.right = right; this.left = left;}
 
  Object eval(ValueEnv env) throws RuntimeError {
    int l, r =0;
    try {
      r = (Integer) right.eval(env);
      l = (Integer) left.eval(env);
    } catch (ClassCastException exn) {
      throw new RuntimeError("Less then equal to requires BOOL first operand");
    }
    return (r <= l) ;
  }
}
  ...
  public static void main(String argv[]) {
    ...
    Exp e = new LetExp("y",
                       new NotExp(new AndExp (new VarExp("true"),
                                              new VarExp("false"))),
                       new IfExp(new LesEqu( new NumExp(2), 
                                             new NumExp(5)),
                                 new LetExp ("y",
                                             new NumExp(21),
